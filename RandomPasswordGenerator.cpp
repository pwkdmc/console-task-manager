...
void PasswordAnalysis(std::string& password) {
	...
	if (complexity < 18) {
		std::cout << "Надежность пароля: слабый" << std::endl;
	}
	else if (complexity < 30) {
		std::cout << "Надежность пароля: средний" << std::endl;
	}
	else {
		std::cout << "Надежность пароля: сильный" << std::endl;
	}
}
...
void RandomPasswordGenerator() {
	...
	do {
		std::cout << std::endl
			<< "== ГЕНЕРАТОР СЛУЧАЙНЫХ ПАРОЛЕЙ ==" << std::endl;
		int len;
		do {
			std::cout << "Введите длину пароля (от 6 до 32 символов):" << std::endl;
			std::cin >> len;
		...
		do {
			...
			std::cout << "1. Цифры" << std::endl << "2. Прописные буквы" << std::endl << "3. Строчные буквы" << std::endl << "4. Спецсимволы" << std::endl;
			std::cout << "Введите типы символов (число длиной от 1 до 4, содержащее неповторяющиеся цифры от 1 до 4):" << std::endl;
			std::cin >> types_num;
			std::cin.ignore(10000, '\n');
			if (std::cin.fail())
			{
